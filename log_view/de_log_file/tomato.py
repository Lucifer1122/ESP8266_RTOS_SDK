...
class CameraUser(threading.Thread):
    def __init__(self, addr, name, type="cav"):
    ...
    def run(self):
        ...
        if "c" in self.type:
            control = CameraCotrolChannel(self.addr, self.name)
            control.start()
        if "v" in self.type:
            video = CameraVideoChannel(self.addr, self.name)
            video.start()
        if "a" in self.type:
            audio = CameraAudioChannel(self.addr, self.name)
            audio.start()

class CameraCotrolChannelHandel(threading.Thread):
...
class CameraCotrolChannel(threading.Thread):
    def __init__(self, addr, name = 123, type = 1):
        threading.Thread.__init__(self)
...
#        print "rHead: ", rHead#, content
    ...
    def run(self):
        ...
        handle = CameraCotrolChannelHandel(sock)
        handle.start()
#        time.sleep(3)
...
class CameraVideoChannel(CameraCotrolChannel):
    def __init__(self, addr, name = 123, type = 2):
        CameraCotrolChannel.__init__(self, addr, name, type)
    def run(self):
        sock = self.sock
        addr = self.addr
...
class CameraAudioChannel(CameraCotrolChannel):
    def __init__(self, addr, name = 123, type = 3):
        CameraCotrolChannel.__init__(self, addr, name, type)
    def run(self):
        sock = self.sock
        addr = self.addr
